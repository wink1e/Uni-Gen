...
def predict_each_epoch(model: torch.nn.Module, dataloader_predict: NNDataLoader,
    ...
    with torch.no_grad():
        for batch_i, batch in enumerate(dataloader_predict):
            ...
            loss = criterion(decoder_distance, net_input['target_src_distance'].to(device)) + 2 * \
                   criterion(decoder_coord, net_input['target_src_coord'].to(device))
            loss_data += loss.data
            show_batch_loss_data = loss.data
            batch_time = time.time() - batch_begin
    loss_data /= num_batch
...
def train_model(lr: float, lr_patience: int,
    ...
    ###
    parser = ArgumentParser("DDP training process")
    ...
    models = DDP(models, device_ids=[args.local_rank], output_device=args.local_rank)
